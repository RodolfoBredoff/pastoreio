...
export async function POST(
  ...
  try {
    ...
    const meeting = await queryOne<{ id: string; group_id: string; attendance_list_mode: string | null }>(
      `SELECT id, group_id, attendance_list_mode FROM meetings WHERE id = $1`,
      [meetingId]
    );
    ...
    const id = uuidv4();
    await query(
      `INSERT INTO attendance_list_public_entries (id, meeting_id, first_name, last_name, email, phone)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [id, meetingId, fn, ln, em, ph]
    );
